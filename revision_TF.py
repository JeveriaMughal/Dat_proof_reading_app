...
def sentence_form(lines_done,df):
    ...
    comment_all=df["comment"].values.tolist()
    ...
    if lines_done < 0 :
        ...
        comment_new=st.text_input("comment",value=default)
        if comment_new =="":
            comment=comment_all[lines_done]
        else:
            comment=comment_new
...
def app():
    ...
    if lines_done<=0:
        ...
        line_number=st.selectbox("choose index",df)
